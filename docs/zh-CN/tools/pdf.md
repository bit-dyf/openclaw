...
仅当 OpenClaw 能为智能体解析出支持 PDF 的模型配置时，才会注册该工具：
...
- 如果未提供 PDF 输入，工具会报错。
- `pages` 解析为基于 1 的页码，去重、排序并限制在配置的最大页数范围内。
...
1. 从所选页面提取文本（最多 `agents.defaults.pdfMaxPages` 页，默认 `20`）。
2. 如果提取的文本长度低于 `200` 字符，则将所选页面渲染为 PNG 图像并包含在内。
3. 将提取的内容加上提示发送到所选模型。

...
read_when:
  ...
  - 你想在无需本地 shell 访问的情况下进行远程分析
  - 你想将 x_search 或 web_search 与远程 Python 分析结合使用
summary: code_execution — 通过 xAI 运行沙箱隔离的远程 Python 分析
title: 代码执行
x-i18n:
...
当你需要本地文件、shell、仓库或配对设备时，**不要**使用它。那种情况请使用 [`exec`](/tools/exec)。

## 设置

你需要 xAI API 密钥。以下任意一种均有效：
...
工具内部接受一个 `task` 参数，因此智能体应在一个提示中发送完整的分析请求和任何内联数据。

## 限制

- 这是远程 xAI 执行，不是本地进程执行。
- 应将其视为临时分析，而非持久化 notebook。
- 不要假设可以访问本地文件或工作区。

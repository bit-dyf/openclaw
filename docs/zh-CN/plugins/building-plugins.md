...
title: "构建插件"
sidebarTitle: "入门指南"
summary: "几分钟内创建您的第一个 OpenClaw 插件"
read_when:
  - 您想要创建新的 OpenClaw 插件
  - 您需要插件开发的快速入门
  - 您正在向 OpenClaw 添加新的渠道、提供商、工具或其他功能
x-i18n:
  generated_at: "2026-03-29T11:37:15Z"
  model: claude-sonnet-4-6
  provider: pi
  source_hash: 64feb49a39d8f45e88ab0cf4d9763cef1062824750aafc4e6434e122ae9f0096
  source_path: docs/plugins/building-plugins.md
  workflow: 15
---

# 构建插件

插件通过新功能扩展 OpenClaw：渠道、模型提供商、语音、图像生成、网络搜索、智能体工具或任何组合。

您不需要将插件添加到 OpenClaw 存储库。发布到 [ClawHub](/tools/clawhub) 或 npm，用户可以使用 `openclaw plugins install <package-name>` 安装。OpenClaw 首先尝试 ClawHub，然后自动回退到 npm。

## 先决条件

- Node >= 22 和包管理器（npm 或 pnpm）
- 熟悉 TypeScript（ESM）
- 对于仓库内插件：已克隆存储库并完成 `pnpm install`
...
<CardGroup cols={3}>
  ...
  <Card title="工具 / 钩子插件" icon="wrench">
    注册智能体工具、事件钩子或服务 — 继续下面
  </Card>
</CardGroup>

## 快速入门：工具插件

本演练创建一个注册智能体工具的最小插件。渠道和提供商插件有上面链接的专用指南。
...
<Steps>
  <Step title="创建包和清单">
    ...
    每个插件都需要清单，即使没有配置。请参阅[清单](/plugins/manifest)获取完整架构。
  ...
  <Step title="编写入口点">
    ...
    `definePluginEntry` 用于非渠道插件。对于渠道，使用 `defineChannelPluginEntry` — 请参阅[渠道插件](/plugins/sdk-channel-plugins)。有关完整入口点选项，请参阅[入口点](/plugins/sdk-entrypoints)。
...
| 智能体工具            | `api.registerTool(...)`                       | 下面                                                                                    |
| 自定义命令            | `api.registerCommand(...)`                    | [入口点](/plugins/sdk-entrypoints)                                                      |
| 事件钩子              | `api.registerHook(...)`                       | [入口点](/plugins/sdk-entrypoints)                                                      |
| HTTP 路由             | `api.registerHttpRoute(...)`                  | [内部](/plugins/architecture#gateway-http-routes)                                       |
| CLI 子命令            | `api.registerCli(...)`                        | [入口点](/plugins/sdk-entrypoints)                                                      |

有关完整注册 API，请参阅 [SDK 概览](/plugins/sdk-overview#registration-api)。

要记住的钩子保护语义：

- `before_tool_call`：`{ block: true }` 是终止的，并停止较低优先级的处理程序。
- `before_tool_call`：`{ block: false }` 被视为无决定。
- `before_tool_call`：`{ requireApproval: true }` 暂停智能体执行，并通过执行批准覆盖层、Telegram 按钮、Discord 交互或任何渠道上的 `/approve` 命令提示用户批准。
- `message_sending`：`{ cancel: true }` 是终止的，并停止较低优先级的处理程序。
- `message_sending`：`{ cancel: false }` 被视为无决定。

`/approve` 命令通过自动回退处理执行和插件批准。插件批准转发可以通过配置中的 `approvals.plugin` 独立配置。
...
有关完整子路径参考，请参阅 [SDK 概览](/plugins/sdk-overview)。
...
1. 在 [openclaw/openclaw](https://github.com/openclaw/openclaw/releases) 上观察 GitHub 发布标签，并通过 `Watch` > `Releases` 订阅。Beta 标签看起来像 `v2026.3.N-beta.1`。您还可以为官方 OpenClaw X 帐户 [@openclaw](https://x.com/openclaw) 打开通知以获取发布公告。
2. 尽快针对 beta 标签测试您的插件。稳定版之前的窗口通常只有几个小时。
3. 在测试后，在 `plugin-forum` Discord 频道的插件线程中发布 `all good` 或什么坏了。如果您还没有线程，请创建一个。
4. 如果某些内容损坏，请打开或更新标题为 `Beta blocker: <plugin-name> - <summary>` 的问题，并应用 `beta-blocker` 标签。将问题链接放在您的线程中。
5. 打开标题为 `fix(<plugin-id>): beta blocker - <summary>` 的 PR 到 `main`，并在 PR 和 Discord 线程中链接问题。贡献者无法标记 PR，因此标题是维护者和自动化的 PR 端信号。具有 PR 的阻止程序被合并；没有 PR 的阻止程序可能仍然会发布。维护者在 beta 测试期间观察这些线程。
...
<CardGroup cols={2}>
  ...
  <Card title="运行时帮助器" icon="settings" href="/plugins/sdk-runtime">
    通过 api.runtime 进行 TTS、搜索、子智能体
  </Card>
